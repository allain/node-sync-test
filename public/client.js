...
app.on('keyup', '#personal-todos .new-todo').pipe(writable({objectMode: true}, function(e) {
  if (e.keyCode != 13) return;
  e.preventDefault();
...
app.on('click', '#personal-todos .add-todo').pipe(writable({objectMode: true}, function(e) {
...
app.on('click', '#personal-todos .completed').pipe(writable({objectMode: true}, function(e) {
